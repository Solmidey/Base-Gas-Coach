...
const FALLBACK_TOOLS: ProtocolTip[] = [
  ...
  },
  {
    ...
    description: "Lend/borrow on Base and put idle assets to work.",
  },
  {
    name: "Beefy",
...
export default function GasAnalyzer({ address }: GasAnalyzerProps) {
  ...
  const handleComingSoonClick = () => {
    setComingSoon(
      "Right now we only analyze the last 2 months (within free explorer limits). 3, 6 and 12 month views are coming soon.",
    );
  };
  ...
  if (!address) {
    return (
      <div className="mt-6 rounded-xl border border-slate-800 bg-slate-950/60 p-4 text-sm text-slate-300">
        Connect a Base wallet or paste an address above to see how your gas
        habits affect you, and get simple, clear coaching to improve.
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="mt-6 rounded-xl border border-slate-800 bg-slate-950/60 p-4 text-sm text-slate-300">
        Analyzing your Base activity for the last {periodLabel}…
      </div>
    );
  }
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      {comingSoon && (
        <p className="text-[11px] text-amber-300/90">{comingSoon}</p>
      )}
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4 text-xs text-slate-200">
        {isEmpty ? (
          ...
          <>
            ...
            <p className="mb-1">
              • Zero-value / approval txs ratio:{" "}
              {(data.zeroValueRatio * 100).toFixed(1)}%
            </p>
            <p className="mt-2 text-slate-400">
              Use this to see where gas is leaking versus where it helps you
              earn in the last {periodLabel}.
            </p>
          </>
        )}
      </div>
